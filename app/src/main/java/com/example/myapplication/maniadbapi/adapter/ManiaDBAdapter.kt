...
class ManiaDBAdapter: ListAdapter<Item, ManiaDBAdapter.MyViewHolder>(diffUtil) {

    // 생성된 뷰 홀더에 값 지정
    inner class MyViewHolder(private val binding: SongItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item : Item) {
            binding.item = item
        }
    }
